...
for h in range(12):
    ...
    ax.scatter(neww_X[:,0], neww_X[:,1], [0,0,0,0], linewidths=1,color='yellow')
    # vocab=op
    ...
    for k, word in enumerate(jp):
        ax.text(neww_X[k,0]+0.01, neww_X[k,1]+0.01, 0, word, zdir=None)

    # calculate potential
    year = pd.DataFrame({data.iloc[h, 0] : data.iloc[h, 1:]})
    year['pot'] = -np.log(year.iloc[:, 0].astype(np.float64)/100)

    # bar graph code
    x, y, z = neww_X[:,0], neww_X[:,1], year.iloc[:-1, 0].astype(np.float64).tolist()
    ...
    ax.set_title(data.iloc[h, 0], size=20)
    # h += 1
    ...
    ax.contour3D(X_grid, Y_grid, Z_grid, levels=20, cmap=cm.coolwarm)

    # ax.set_zlim(0, 45)

    ax.view_init(elev=25, azim=280, roll=0)

    plt.xticks([-0.2, -0.1, 0, 0.1, 0.2])
    plt.yticks([-0.2, -0.1, 0, 0.1, 0.2])

    ax.set_xlabel("主成分1", size=12)
    ax.set_ylabel("\n主成分2", size=12)
    ax.set_zlabel("\n回答者の割合 (%), \nポテンシャル*10", size=15)

    plt.savefig('embedding/fig_corrected/%s.png' %data.iloc[h, 0], dpi=300)
