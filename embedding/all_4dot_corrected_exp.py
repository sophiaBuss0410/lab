...
def plotting(vertex):
    ...
    for i in range(3):
        for j in range(4):
            ...
            year1979['pot'] = 10*np.log(year1979.iloc[:, 0].astype(np.float64)/100)
            ...
            ## surface plot code
            # border condition            
            dum = [-1*vertex, vertex, -1*vertex, vertex]
            dumy = [-1*vertex, vertex, vertex, -1*vertex]
            ...
            zz = year1979.iloc[:-1, 1]
            zz = zz.to_list()
            for m in range(len(dum)):
                zz.append(10*np.log(5/100))
            ...
            X_grid, Y_grid = np.mgrid[-1*vertex:vertex:50j, -1*vertex:vertex:50j]
    ...
    plt.savefig('embedding/all_4dots_corrected_%d.png' %(10*vertex), bbox_inches='tight', pad_inches=0.5)
    # plt.show()
